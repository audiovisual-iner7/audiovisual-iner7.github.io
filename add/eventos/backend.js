...
/**
 * Registra un nuevo evento en la hoja "EVENTOS_REG".
 * @param {Object} data - Los datos del evento enviados desde el frontend.
 * @returns {ContentService} - Una respuesta JSON indicando éxito o fracaso.
 */
function registrarEvento(data) {
  try {
    ...
    // Mapa de los tipos de evento a sus columnas (F=6, G=7, ..., L=12)
    const tipoEventoColumnMap = {
      'SESION_GENERAL': 6,
      'SESION_ENFERMERIA': 7,
      'CIENI': 8,
      'EDUCACION': 9,
    ...
    // Mapa de los servicios (checkboxes) a sus columnas (O=15, P=16, ..., AN=40)
    const serviciosColumnMap = {
      'CARTEL': 15, 'POSTAL_DIFUSION': 16, 'REEL_VIDEO': 17, 'ENCABEZADO_FORMULARIOS': 18,
      'PROGRAMA_PORTADA_INTERIOR': 19, 'BANNER_VERTICAL_PODIUM': 20, 'PROGRAMA_PANTALLA_LOBBY': 21,
      'CORTINILLA_GENERAL': 22, 'CORTINILLA_ANIMADA_GENERAL': 23, 'CORTINILLA_ASISTENCIA': 24,
      'CORTINILLA_INSCRIPCION_EVAL_INICIAL': 25, 'CORTINILLA_EVAL_CALIDAD_CONOCIMIENTOS': 26,
      'SUPERS_SESION_GENERAL': 27, 'SUPERS_ORDEN_DIA': 28, 'SUPERS_PROGRAMA': 29, 'QR_BANDERIN': 30,
      'QR_SESION': 31, 'QR_INSCRIPCION_EVAL_INICIAL': 32, 'QR_EVAL_CALIDAD_CONOCIMIENTOS': 33,
      'DOC_PREGUNTAS_SESIONES': 34, 'DOC_PREGUNTAS_EVENTO': 35, 'VIDEO_INTRO_PC': 36,
      'CORTINILLA_PROX_EVENTOS': 37, 'TRANSMISION_YOUTUBE': 38, 'TRANSMISION_ZOOM': 39, 'APOYO_TECNICO': 40
    };
    
    // Preparamos un array para la nueva fila. 40 columnas (A hasta AN).
    const newRow = new Array(40).fill('');

    // Asignar valores a las columnas correspondientes
    newRow[1] = data.noEvento || ''; // Columna B
    newRow[3] = data.fechaInicio;   // Columna D
    newRow[4] = data.fechaFin;      // Columna E
    newRow[12] = data.nombreEvento; // Columna M
    newRow[13] = data.sede;         // Columna N

    // Poner 'x' en la columna del tipo de evento
    const tipoEventoCol = tipoEventoColumnMap[data.tipoEvento];
    if (tipoEventoCol) {
      newRow[tipoEventoCol - 1] = 'x';
    }

    // Poner 'R' para cada servicio requerido
    for (const servicio in data.servicios) {
      if (data.servicios[servicio] === true) { // Si el checkbox estaba marcado
        const servicioCol = serviciosColumnMap[servicio];
        if (servicioCol) {
          newRow[servicioCol - 1] = 'R';
        }
      }
    }
    ...
    return ContentService
      .createTextOutput(JSON.stringify({ success: true, message: 'Evento registrado con éxito.' }))
      .setMimeType(ContentService.MimeType.JSON);
