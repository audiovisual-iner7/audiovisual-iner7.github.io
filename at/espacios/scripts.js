document.addEventListener('DOMContentLoaded', function () {
    ...
    function sendDataToGoogle(data) {
        const scriptURL = 'https://script.google.com/macros/s/AKfycbx3TIV78su8GSkAmxq15Ozcz0l37jEN-u76zjYuVVZkbewnMyx8Qw--rwukOg617B_7DA/exec';
        ...
        fetch(scriptURL, {
            ...
            .then(res => {
                if (res.result === 'success') {
                    ...
                    statusMessage.className = 'text-green-600 font-bold p-2 bg-green-50 rounded border border-green-200';
                    
                    // 1. Limpiar el formulario (inputs y selects)
                    form.reset();

                    // 2. Ocultar el selector de días (usando la clase de Tailwind, no style)
                    // Usamos la variable correcta 'diasSemanaContainer'
                    const diasSemanaContainer = document.getElementById('diasSemanaContainer');
                    if (diasSemanaContainer) {
                        diasSemanaContainer.classList.add('hidden');
                        // Desmarcar checkboxes visualmente
                        const inputs = diasSemanaContainer.querySelectorAll('input');
                        inputs.forEach(i => i.checked = false);
                    }

                    // 3. NO ocultamos el Zoom (zoomContainer) porque pediste que siempre sea visible.
                    // Al hacer form.reset(), el checkbox de zoom se desmarcará solo.

                    // 4. Ocultar campos "OTRO" si quedaron abiertos
                    document.getElementById('otroTipoEventoContainer').classList.add('hidden');
                    document.getElementById('otroTipoActividadContainer').classList.add('hidden');
                    const reqOtroTexto = document.getElementById('req_otro_texto');
                    if (reqOtroTexto) reqOtroTexto.classList.add('hidden');

                } else {
                    // Si el servidor dice "error" (aunque la petición HTTP fue 200)
